...
export default function LearningPage() {
  ...
  return (
    <Shell>
      ...
      {error && <p className="mb-4 rounded-md bg-dangerSurface px-4 py-3 text-sm text-danger">{error}</p>}
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_360px]">
        <div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {courses.map((course) => (
              <article key={course._id} className="flex flex-col card card-pad">
                <div className="flex items-start justify-between gap-2">
                  <span className="rounded bg-brand/10 px-2 py-1 text-xs font-semibold uppercase text-brand">
                    {course.category}
                  </span>
                  {course.status !== "approved" && (
                    <span className="rounded bg-secondary px-2 py-1 text-xs font-semibold uppercase text-muted">
                      {course.status}
                    </span>
                  )}
                </div>
        ...
        {canPublish && (
          <form onSubmit={submit} className="h-fit card card-pad">
